...
const [addingShow , setAddingShow]=useState(false)
  ...
  const handleDateTimeAdd = () => {
    ...
    if (!date || !time) return;   // ✅ fixed condition
 ...
 const handleSubmit = async()=>{
  ...
  return toast.error('Please select movie, date, time and enter show price')
}
      ...
      <div className='overflow-x-auto pb-4'>
        <div className='group flex flex-wrap gap-4 mt-4 w-max'>
          {nowPlayingMovies.map((movie) => (
            <div
              key={movie.id}
              className="relative max-w-33 cursor-pointer group-hover:not-hover:opacity-40 hover:translate-y-1 transition duration-300"
